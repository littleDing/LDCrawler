...
## Output: 	new linkbase file(link sogourank)
function addSogouRank(){
local site=$1; if [ "x$1" = "x"  ]; then return; fi; 
local n=$2; if [ "x$2" = "x"  ]; then n=20; fi; 
local second=$3; if [ "x$3" = "x" ]; then second=1; fi
local tmp=${TMP}/addSogouRank.${site}; rm -rf ${tmp}; mkdir -p ${tmp}
	...
	cd ${tmp}
		...
		output=${tmp}/${site}.sogourank
		rm ${output} 2>/dev/null
		for file in `ls` ; 
		do	
			cat ${tmp}/${file} | ${SOGOU_RANK}  >>${output} 
			sleep ${second}s	
		done
		cp ${output} ${LINKBASE}
	cd ${current};
